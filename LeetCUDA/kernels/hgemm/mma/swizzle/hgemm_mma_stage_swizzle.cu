...
#pragma unroll
  ...
  #pragma unroll
      for (int i = 0; i < WARP_TILE_M; i++) {
        ...
        int lane_smem_a_k = (lane_id / 16) * 8;
        uint32_t lane_smem_a_ptr = (smem_a_base_ptr + 
            (smem_sel * s_a_stage_offset + lane_smem_a_m * (BK + A_PAD) + swizzle_permuted_A_j<MMA_K>(lane_smem_a_m, lane_smem_a_k)) * sizeof(half));
        LDMATRIX_X4(RA[reg_store_idx][i][0], RA[reg_store_idx][i][1],
                  RA[reg_store_idx][i][2], RA[reg_store_idx][i][3],
                  lane_smem_a_ptr);
      }
    ...
    #pragma unroll
      for (int i = 0; i < WARP_TILE_M; i++) {
        ...
        int lane_smem_a_k = (lane_id / 16) * 8;
        uint32_t lane_smem_a_ptr = (smem_a_base_ptr + 
            (smem_sel_reg * s_a_stage_offset + lane_smem_a_m * (BK + A_PAD) + swizzle_permuted_A_j<MMA_K>(lane_smem_a_m, lane_smem_a_k)) * sizeof(half));
        LDMATRIX_X4(RA[reg_store_idx][i][0], RA[reg_store_idx][i][1],
                  RA[reg_store_idx][i][2], RA[reg_store_idx][i][3],
                  lane_smem_a_ptr);
      }
  ...
  #pragma unroll
    ...
    #pragma unroll
      for (int i = 0; i < WARP_TILE_M; i++) {
        ...
        uint32_t lane_smem_a_ptr = (smem_a_base_ptr + s_a_mma_k_store_offset * sizeof(half) +
            (stage_sel * s_a_stage_offset + lane_smem_a_m * (BK + A_PAD) + swizzle_permuted_A_j<MMA_K>(lane_smem_a_m, lane_smem_a_k)) * sizeof(half));
        LDMATRIX_X4(RA[reg_store_idx][i][0], RA[reg_store_idx][i][1],
                  RA[reg_store_idx][i][2], RA[reg_store_idx][i][3],
                  lane_smem_a_ptr);
      }
    ...
    #pragma unroll
      ...
      // load next k iters to reg buffer
