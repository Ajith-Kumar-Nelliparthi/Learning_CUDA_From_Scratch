...
__global__ void gelu_f32x4_kernel(float *x, float *y, int N) {
    ...
    rer_y.x = GELU_OPS(reg_x.x);
    rer_y.y = GELU_OPS(reg_x.y);
    rer_y.z = GELU_OPS(reg_x.z);
    rer_y.w = GELU_OPS(reg_x.w);
...
__global__ void gelu_f16x8_pack_kernel(half *x, half *y, int N) {
    ...
    // temporary registers to hold the unpacked values
    half pack_x[8], pack_y[8]; // 8*16 bits = 128 bits
    // reinterpret as float4 and load 128 bits (4*32 bits) at once
    ...
    // reinterpret as float4 and store 128 bits (4*32 bits) at once
    if ((idx + 7) < N) {
...
#define TROCH_BINDING_COMMON_EXTENSION(func) \
