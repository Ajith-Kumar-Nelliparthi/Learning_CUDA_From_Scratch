...
__global__ void transpose(const float* __restrict__ in, int rows, int cols, float *out){
    int x = blockIdx.x * TILE_SIZE + threadIdx.x; // column index
    int y = blockIdx.y * TILE_SIZE + threadIdx.y; // row index

    if (x < cols && y < rows){
        out[x * rows + y] = in[y * cols + x];  // formula index = row * numcols + col
    }
    // input index = in[y * cols + x] / output index = out[x * rows + y]
}
...
int main(){
    ...
    transpose<<<gridSize, blockSize>>>(d_in, rows, cols, d_out);
